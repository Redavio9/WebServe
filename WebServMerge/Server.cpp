...
/*   Updated: 2024/05/19 12:26:44 by rarraji          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void Server::checkResponse(int socket, std::string host , std::string port)
{
    ...
    for (size_t i = 0; i < this->servers.size(); i++)
    {
        ...
        if(this->servers[i].get_host().compare(host) == 0 && this->servers[i].get_port().compare(port) == 0)
        {
            ...
            std::cout << "url a9dim: " << mapinfo[socket].request.response.url << std::endl;
        ...
        {
            int j = 0;
            std::cout << "isiga irbi file : " << S_ISDIR(fileStat.st_mode) << " | " << tmp1 << std::endl;
            // std::cout << "LOCATION : " << mapinfo[socket].request.url  << std::endl;
